...
inline void vec3_rkf45(vec6 initial, double initTime, double deltaTime, double tolerance, vec6(*fun)(double, vec6),vector<state>* stateOutput) {
	...
	double r1 = (1.0 / 360.0); double r3 = (-128.0 / 4275.0); double r4 = (-2197.0 / 75240.0); double r5 = (1.0 / 50.0); double r6 = (2.0 / 55.0);
	...
	while (t + h < initTime + deltaTime) {
		...
		//err = len(r1*k1+r3*k3+r4*k4+r5*k5+r6*k6);
		err = len(z1 - y1);
		s = sqrt(sqrt((tolerance*h)/(2*len(z1-y1))));
