...
public:
	...
	void updateModelMat() {
		this->model = TransMat(position) * rotation * ScaleMat(scale);
	}
	void rotate(double angle, vec3 axis) {
		this->rotation = qtomat4(quaternion(angle, axis)) * rotation;
		this->updateModelMat();
	}
...
public:
	Skybox(string filename) : Skybox(filename, vec3{ 0.0,0.0,0.0 }) {
	}
